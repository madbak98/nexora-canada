...
function QuickView({ service }: { service: Service }) {
  return <section className="section section-light" id="quick-view"><LightSectionGrid /><div className="container"><div className="section-heading split-heading"><div><Eyebrow>Quick view</Eyebrow><h2>{service.category} works best when the pieces connect.</h2></div><p>{service.quickIntro}</p></div><div className="quick-grid">{service.quick.map((item, index) => <a href={`#${item.anchor}`} key={item.label} className="quick-card"><span className="quick-number">0{index + 1}</span><div><h3>{item.label}</h3><p>{item.detail}</p></div><span className="quick-arrow">↘</span></a>)}</div></div></section>;
}
...
function IncludedSection({ service }: { service: Service }) {
  return <section className="section section-light" id="included"><LightSectionGrid /><div className="container"><div className="section-heading split-heading"><div><Eyebrow>What we deliver</Eyebrow><h2>{service.includedTitle}</h2></div><p>{service.includedIntro}</p></div><div className="deliver-grid">{service.deliverables.map((item, index) => <article className="deliver-card" key={item.title}><span className="deliver-number">0{index + 1}</span><h3>{item.title}</h3><p>{item.detail}</p></article>)}</div></div></section>;
}
...
function FAQSection({ service }: { service: Service }) {
  return <section className="section section-light" id="faq"><LightSectionGrid /><div className="container faq-layout"><div><Eyebrow>Questions</Eyebrow><h2>Before you start.</h2><p className="faq-intro">A few useful answers to help you decide whether this is the right next conversation.</p></div><div className="faq-list">{service.faq.map((item, index) => <details key={item.question} open={index === 0}><summary>{item.question}<span>+</span></summary><p>{item.answer}</p></details>)}</div></div></section>;
}
...
export default function ServicePage({ params }: { params: { slug: string } }) {
  ...
  return <SiteShell><main>
    <section className="service-hero"><NexoraFerrofluid /><div className="container service-hero-grid"><div className="service-hero-copy"><div className="breadcrumbs"><Link href="/">Home</Link><span>/</span><span>{service.category}</span></div><Eyebrow light>{service.category}</Eyebrow><h1>{service.title}</h1><p className="hero-lead">{service.description}</p><div className="button-row"><ArrowButton href="/start-project">Start a project</ArrowButton><ArrowButton href="#quick-view" secondary>See what’s included</ArrowButton></div><div className="service-proof"><span>Clear scope</span><span>Connected direction</span><span>Built to improve</span></div></div><ServiceVisual service={service} /></div></section>
